...
class PurchaseRequisition(models.Model):
    ...
    def _set_note1(self):
        ...
        if comment:
            self.note1 = comment.get_value(self.vendor_id.id)

    @api.onchange("comment_template2_id")
    def _set_note2(self):
        comment = self.comment_template2_id
        if comment:
            self.note2 = comment.get_value(self.vendor_id.id)

    @api.onchange("vendor_id")
    def onchange_vendor_id_purchase_comment(self):
        if self.vendor_id:
            comment_template = self.vendor_id.property_comment_template_id
